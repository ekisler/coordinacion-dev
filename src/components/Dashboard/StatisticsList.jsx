...
const StatisticsList = () => {
  ...
  const stats = useMemo(() => {
    // NUEVO: Filtro de profesores reales (isTeacher !== false)
    const strictlyTeachers =
      teachers?.filter((t) => t?.userId?.roles?.isTeacher !== false) || [];

    // 1. Distribución por Género
    const genderDist = [
      {
        name: "Masculino",
        value: students?.filter((s) => s.sex === "Masculino").length || 0,
      },
      {
        name: "Femenino",
        value: students?.filter((s) => s.sex === "Femenino").length || 0,
      },
    ];
    ...
    const gradeData = Object.keys(gradeMap).map((key) => ({
      name: key,
      value: gradeMap[key],
    }));
    ...
    const attendanceData = Object.keys(attendanceMap).map((key) => ({
      name: key,
      count: attendanceMap[key],
    }));

    return { genderDist, gradeData, attendanceData, strictlyTeachers };
  }, [students, teachers, attendances, viewMode]);
  ...
  return (
    <Box p={{ base: 4, md: 6 }}>
      ...
      >
        <VStack align="start" spacing={1}>
          ...
          <Text color={mutedColor} fontSize="sm">
            Análisis detallado de población y rendimiento
          </Text>
        </VStack>
      ...
      <SimpleGrid columns={{ base: 1, md: 3 }} spacing={6} mb={8}>
        ...
        <StatCard
          title="Profesores"
          count={stats.strictlyTeachers.length}
          icon={GiTeacher}
          color="purple.500"
        />
        <StatCard
        ...
        <Card bg="teal.500" color="white" borderRadius="xl" boxShadow="lg">
          ...
          >
            ...
            <Text fontSize="sm" mb={4}>
              Actualmente contamos con una población activa de{" "}
              <b>{students?.length + teachers?.length}</b> integrantes entre
              docentes y alumnos.
            </Text>
            <Button
              size="sm"
  ...
  >
    <CardBody>
      <Flex align="center" justify="space-between">
        <Stat>
          <StatLabel color="gray.500" fontWeight="bold" fontSize="sm">
            {title}
          </StatLabel>
